...
# ---------------------------------------------------------------------------
# Fuzzy matching — fast early-exit scan
# ---------------------------------------------------------------------------

def isSomething(inp: str, keyword_list, accuracy: float) -> bool:
    """Return True if `inp` fuzzy-matches any entry in `keyword_list`."""
    if not keyword_list or not inp:
        return False
    if isinstance(keyword_list, str):
        return Levenshtein.ratio(inp, keyword_list) >= accuracy
    for kw in keyword_list:
        if Levenshtein.ratio(inp, kw) >= accuracy:
            return True
    return False


# ---------------------------------------------------------------------------
# Token extraction (unchanged logic, kept for completeness)
# ---------------------------------------------------------------------------
